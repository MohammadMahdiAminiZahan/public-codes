...
class Environment: def init(self, world): self.world = world self.width = len(world[0]) self.height = len(world)
...
class InferenceAgent: def init(self, env): self.env = env self.knowledge = dict() self.visited = set() self.safe_to_visit = set() self.unsafe = set() self.pit_candidates = defaultdict(set) self.wumpus_candidates = defaultdict(set) self.pit_confirmed = None self.wumpus_confirmed = None self.current = (0, 3)  # bottom-left self.found_gold = False self.path = [self.current]
...
world = [ ['B', 'B', '', ''], ['P', 'B', 'S', ''], ['B', 'GSB', 'S', 'S'], ['', 'S', 'W', 'S'] ]

env = Environment(world) agent = InferenceAgent(env) agent.explore() print("\n🧭 Masir tay shode:", agent.path)
